...
summary: 使用 Python 程式碼範例建立或刪除ONTAP Select叢集時，必須提供一個 JSON 檔案作為腳本的輸入。您可以根據部署計劃複製和修改相應的 JSON 範例。 
---
= 用於建立ONTAP Select叢集的腳本的 JSON
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
使用 Python 程式碼範例建立或刪除ONTAP Select叢集時，必須提供一個 JSON 檔案作為腳本的輸入。您可以根據部署計劃複製和修改相應的 JSON 範例。
